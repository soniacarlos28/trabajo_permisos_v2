...
//PreparedStatement StatementRSQUERY = ConnRSQUERY.prepareStatement("SELECT ID_FUNCIONARIO, INITCAP(APE1) AS APE1, INITCAP(APE2) AS APE2, INITCAP(NOMBRE) AS NOMBRE, ID_FUNCIONARIO, FECHA_INGRESO, ACTIVO, JORNADA, NUMERO_SS, DECODE(TIPO_FUNCIONARIO,'N','Funcionario de la Administraci�n','B','Bomberos','P','POLICIA',' ') as TIPO_FUNCIONARIO  FROM RRHH.PERSONAL_NEW  WHERE ID_FUNCIONARIO = '" + RSQUERY__MMColParam1 + "' AND APE1 LIKE '" + RSQUERY__MMColParam2 + "%' AND APE2 LIKE '" + RSQUERY__MMColParam3 + "%' AND NOMBRE LIKE '" + RSQUERY__MMColParam4 + "%' AND DNI = LPAD('" + RSQUERY__MMColParam5 + "',8,0) AND DNI_LETRA LIKE '" + RSQUERY__MMColParam6 + "%'");
